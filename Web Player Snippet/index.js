...
var sourceNode = context.createMediaElementSource(video); //EQ
...
closeDialogBtn.addEventListener("click", closedDialog); //Transcript Panel
...
function displayCues(track) {
    ...
    for (var i = 0, len = cues.length; i < len; i++) {
        ...
        var clickableTranscriptText = `<div class="cue-container" id="${cue.startTime
            }" role="button" aria-pressed="false" tabindex="0" onclick="jumpToTranscript(${cue.startTime
            });"> <div class="cue-time span">${formatDuration(
                cue.startTime
            )}</div> <div class="cues span">${transcriptText}</div> </div>`;
